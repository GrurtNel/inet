...
public class GenericResourceLoader {
     ...
     * </p>
     *
     * @param pathToResource path relative to the calling context. For example:
     * loadResource(/app.properties, false) for an
     * EAR/APP-INF/classes/app.properties
     * @param useContextClassLoader if <code>true</code> then use the current
     ...
     * @see java.lang.Thread#getContextClassLoader()
     */
    private static InputStream loadResource(String pathToResource, ServletContext servletContext, boolean useContextClassLoader) {
        if (debug)
            say("loadResource with pathToResource : " + pathToResource + " useContextClassLoader " + useContextClassLoader);
        if (debug)
            say("using classloader : " + GenericResourceLoader.class.getClassLoader().getClass().getName());
        return servletContext.getResourceAsStream(pathToResource);
    }
     ...
     *
     */
    public static InputStream loadResource(String pathToResource, ServletContext servletContext) {
        return loadResource(pathToResource, servletContext, false);
    }
